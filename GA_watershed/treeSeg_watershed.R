...
SEGMENT <- function(las, ground_truths, vals)
{
  ...
  #    tol = 1, 
  #    ext = 1)
  cat("watershed(chm, th_tree = ", as.character(args[1]), ", tol = ", as.character(args[2]), ", ext = ", as.character(args[3]),")")
    
  algo = watershed(chm = CHM, 
                   th_tree = as.double(args[1]), 
                   tol = as.double(args[2]), 
                   ext = as.double(args[3]))
...
#cat(ls_str, file="C:/Users/Lee/Desktop/CS776-GA-Project/GA_watershed/ls.txt", sep="\n", append=TRUE)
cat(ls_str, file="C:/Users/Lee/Desktop/CS776-GA-Project/watershed_ls.txt", sep="\n", append=TRUE)
